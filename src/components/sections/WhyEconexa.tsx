  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-white via-slate-50 to-white">
      ...
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-bl from-orange-50 to-transparent rounded-full blur-[140px] opacity-50" />
      ...
      <Container>
        <div className="grid gap-16 lg:grid-cols-2 lg:items-center">
          ...
          >
            {/* Glow - Light Version */}
            <div className="absolute -inset-4 bg-gradient-to-tr from-[rgb(var(--brand))]/15 to-[rgb(var(--brand-2))]/15 rounded-3xl blur-2xl opacity-40" />
            
            {/* Main Image - Light Version */}
            ...
            {/* Floating Badge - Light Version */}
            <div className="absolute -top-4 -right-4 rounded-xl bg-[rgb(var(--brand))] px-4 py-2.5 shadow-lg shadow-[rgb(var(--brand))]/30">
              <p className="text-sm font-bold text-white">100% Online</p>
            </div>
          </motion.div>

          {/* RIGHT: Content - Light Version */}
          <motion.div
            initial={{ opacity: 0, x: 40 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.7, ease }}
          ...
          >
            ...
            <div className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 shadow-sm mb-6">
              <div className="h-2 w-2 rounded-full bg-[rgb(var(--brand))] animate-pulse" />
              Ventajas Exclusivas
            </div>
              ...
              {features.map((f, i) => {
                ...
                return (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    whileHover={{ y: -2 }}
                    className="group flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition-all hover:border-[rgb(var(--brand))]/40 hover:shadow-md hover:shadow-[rgb(var(--brand))]/5"
                  >
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[rgb(var(--brand))]/10 group-hover:bg-[rgb(var(--brand))]/20 transition-colors">
                      <Icon className="h-5 w-5 text-[rgb(var(--brand))]" />
                    </div>
                    <div>
                      <h3 className="text-sm font-semibold text-slate-900 group-hover:text-[rgb(var(--brand))] transition-colors">{f.title}</h3>
            ...
            {/* Check List - Light Version */}
            <div className="mt-8 space-y-3">
              {["Actualización constante", "Soporte 24/7", "Comunidad activa"].map((item) => (
                <div key={item} className="flex items-center gap-3">
                  <div className="flex h-5 w-5 items-center justify-center rounded-full bg-[rgb(var(--brand-2))]/10">
                    <CheckCircle className="h-3.5 w-3.5 text-[rgb(var(--brand-2))]" />
                  </div>
            ...
            >
              <a
                href="/cursos"
                className="inline-flex items-center gap-2 rounded-xl bg-[rgb(var(--brand))] px-6 py-3.5 text-base font-semibold text-white shadow-lg shadow-[rgb(var(--brand))]/30 transition-all hover:bg-[rgb(var(--brand))]/90 hover:shadow-xl"
              >
